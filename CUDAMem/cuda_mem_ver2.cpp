...
namespace {
  struct cudaMem : public FunctionPass {
    ...
    void insertChannelInitBlock(Function *func, Instruction *inst) {
      ...
      prefetch_val = cast<InvokeInst>(inst)->getArgOperand(prefetchOprandIndex);
      auto *first_instruction = dyn_cast<llvm::Instruction>(prefetch_val);
      // prefetch_val->getType()->print(rso);
      if(nullptr != first_instruction) {
        // errs() << "\t Instruction Name : " << first_instruction->getOpcodeName() << ":" << rso.str() << "\n";
        errs() << "\t Operand Numbers : " << first_instruction->getNumOperands() << "\n";
      }

      Value* load_val = cast<CallInst>(first_instruction)->getArgOperand(0);
      auto *load_instruction = dyn_cast<llvm::Instruction>(load_val);
      
      if(nullptr != load_instruction) {
        // errs() << "\t Instruction Name : " << load_instruction->getOpcodeName() << ":" << rso.str() << "\n";
        errs() << "\t Operand Numbers : " << load_instruction->getNumOperands() << "\n";
      }
      auto *new_inst = load_instruction->clone();
      ...
      Value* ir_ptr = builder.CreateBitCast(prefetch_val, Type::getInt8PtrTy(context));
      load_instruction->insertBefore(dyn_cast<llvm::Instruction>(ir_ptr));
          ...
          if(inst.getOpcode() == Instruction::Call || inst.getOpcode() == Instruction::Invoke) { 
            ...
            if (isa<InvokeInst>(&inst)) {
              ...
              if(func) {
                ...
                //   }
                // }
                // Function *main_func = BB.getParent();
                // for(BasicBlock &bb : F) {
                //   if(cnt == 2) {
                //     BB.moveAfter(&bb);
                //   }
                ...
                if(fname == kernelName) {
                  ...
                  // }
                  // auto *new_inst = load_instruction->clone();
                  // insertChannelInitBlock(func, &(*prefetch_val), &(*new_inst), &(inst));
                  // insertChannelInitBlock(func, &(inst));
                  insertChannelInitBlock(&F, &inst);
                  // for (auto mod_func = module->begin(); mod_func != module->end(); mod_func++) {
                    // if (mod_func->getName().str() == "main") {
                    //   insertChannelInitBlock(&(*func));
                    // }
                  // }
                }
              }
